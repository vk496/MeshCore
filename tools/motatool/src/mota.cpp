...
static void merkle_leaf(uint8_t out[4], const uint8_t* block, size_t len) { sha256_trunc(out, 4, block, len); }
static void merkle_combine(uint8_t out[4], const uint8_t* left, const uint8_t* right) {
  uint8_t buf[8]; std::memcpy(buf, left, 4); std::memcpy(buf + 4, right, 4); sha256_trunc(out, 4, buf, 8);
}
static void merkle_root(uint8_t out[4], const uint8_t* leaves, uint32_t count) {
  if (count == 0) { std::memset(out, 0, 4); return; }
  if (count == 1) { std::memcpy(out, leaves, 4); return; }
  uint8_t peaks[32][4]; bool valid[32] = {false};
  for (uint32_t i = 0; i < count; i++) {
    uint8_t cur[4]; std::memcpy(cur, leaves + (size_t)i * 4, 4);
    uint32_t level = 0;
    while (valid[level]) { merkle_combine(cur, peaks[level], cur); valid[level] = false; level++; }
    std::memcpy(peaks[level], cur, 4); valid[level] = true;
  }
  int level = 0; while (level < 32 && !valid[level]) level++;
  uint8_t acc[4]; std::memcpy(acc, peaks[level], 4);
  for (int l = level + 1; l < 32; l++) if (valid[l]) merkle_combine(acc, peaks[l], acc);
...
std::string Manifest::hw_id_str() const {
  size_t n = 0; while (n < hw_id.size() && hw_id[n]) n++;
...
std::string parse(const std::vector<uint8_t>& b, Manifest& m) {
  ...
  if (m.block_size_log2 == 0 || m.block_size_log2 > 24 || m.payload_size == 0) return "bad block_size/payload";
...
std::vector<std::string> verify(const std::vector<uint8_t>& b) {
  ...
  if (!e.empty()) { probs.push_back(e); return probs; }   // unparseable: a single, fatal problem
  ...
  uint8_t root[4]; merkle_root(root, leaves, m.block_count);
  if (std::memcmp(root, m.merkle_root.data(), 4) != 0)
    probs.push_back("merkle_root mismatch");
...
FwIdent parse_endf_ident(const std::vector<uint8_t>& img) {
  ...
  size_t n = 0; while (n < HW_ID_LEN && t[ENDF_OFF_HWID + n]) n++;
...
std::vector<uint8_t> ensure_endf(const std::vector<uint8_t>& img, const FwIdent& id,
  ...
  uint8_t u[4]; wr_u32(u, (uint32_t)img.size()); out.insert(out.end(), u, u + 4);
  out.insert(out.end(), body_hash8.begin(), body_hash8.end());
  wr_u32(u, id.fw_version); out.insert(out.end(), u, u + 4);
  wr_u32(u, id.target_id);  out.insert(out.end(), u, u + 4);
...
bool pack_version(const std::string& s, uint32_t& out) {
  uint32_t parts[4] = {0,0,0,0}; int n = 0; bool any = false;
  std::stringstream ss(s); std::string tok;
  while (std::getline(ss, tok, '.') && n < 4) {
    if (tok.empty()) return false;
    for (char c : tok) if (c < '0' || c > '9') return false;
    parts[n++] = (uint32_t)std::strtoul(tok.c_str(), nullptr, 10); any = true;
  }
  if (!any) return false;
...
static std::string detools_delta(const std::string& detools, uint8_t codec,
  ...
  if (!write_file(fb, base_img.data(), base_img.size()) || !write_file(fn, new_img.data(), new_img.size())) {
    ...
    if (codec == CODEC_DETOOLS_SEQUENTIAL)
      ...
      argv = {detools, "create_patch_in_place", "-c", "crle",
              "--memory-size", std::to_string(mem), "--segment-size", std::to_string(seg), fb, fn, fp};
    int rc = run_argv(argv, /*quiet=*/true);          // hide detools' success chatter; errors keep stderr
    if (rc == 127) err = "could not run detools (install it / pass --detools <path>)";
    else if (rc != 0) err = "detools exited with code " + std::to_string(rc);
    else if (!read_file(fp, patch) || patch.empty()) err = "reading detools patch failed";
  }
  unlink(fb); unlink(fn); unlink(fp);
  return err;
}
...
std::string detools_apply_seq(const std::string& detools, const std::vector<uint8_t>& base_img,
  ...
  if (!write_file(ff, base_img.data(), base_img.size()) || !write_file(fpp, patch.data(), patch.size())) {
    ...
    if (rc == 127) err = "could not run detools (install it / pass --detools <path>)";
    else if (rc != 0) err = "detools apply_patch exited with code " + std::to_string(rc);
    else if (!read_file(ft, out) || out.empty()) err = "reading reconstructed image failed";
  }
  unlink(ff); unlink(fpp); unlink(ft);
  return err;
}
...
std::string build(const BuildOpts& o, std::vector<uint8_t>& out, std::string& suggested_name) {
  ...
  if (!is_delta) {
    ...
    std::string e = detools_delta(o.detools, codec, base_img, new_img,
                                  o.inplace_memory, o.inplace_segment, payload);
    if (!e.empty()) return e;
  }
  ...
  uint8_t root[4]; merkle_root(root, leaves.data(), bc);
  ...
  mf[M_OFF_FORMAT_VER] = FORMAT_VER; mf[M_OFF_FLAGS] = flags; mf[M_OFF_HASH_ALGO] = HASH_ALGO_SHA256;
  ...
  // block_size_log2
  { uint32_t v = bs, l = 0; while (v > 1) { v >>= 1; l++; } mf[M_OFF_BLOCK_SIZE_LOG2] = (uint8_t)l; }
  ...
  uint8_t u[4]; wr_u32(u, total); out.insert(out.end(), u, u + 4);
  ...
  char tgt[9]; std::snprintf(tgt, sizeof(tgt), "%08X", target);
  char vbuf[24]; std::snprintf(vbuf, sizeof(vbuf), "%u.%u.%u",
                               (fwver >> 24) & 0xFF, (fwver >> 16) & 0xFF, (fwver >> 8) & 0xFF);
  suggested_name = (hw.empty() ? std::string("fw") : hw) + "_" + tgt + "_v" + vbuf + "_" + kind +
                   "_" + to_hex(root, 4) + ".mota";
  return "";
}
